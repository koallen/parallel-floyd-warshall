...
do
    ...
    for size in 1024 2048 4096
    do
        ./test $size > speedup/coa_$blk_$size.output
        nvprof --analysis-metrics -o profiling/coa_$blk_$size.nvprof ./test $size
    done
done
...
do
    ...
    for size in 1024 2048 4096
    do
        ./test $size > speedup/sm_$blk_$size.output
        nvprof --analysis-metrics -o profiling/sm_$blk_$size.nvprof ./test $size
    done
done
...
do
    ...
    for size in 1024 2048 4096
    do
        ./test $size > speedup/blk_$blk_$size.output
        nvprof --analysis-metrics -o profiling/blk_$blk_$size.nvprof ./test $size
    done
done
